...
function validateProp(prop, val, required) {
   ...
   /* if (!val) {
        console.log("INFO.REQUIRED===="+info.required);
        console.log("REQUIRED===="+required);
        if (info.required && required) {

            //return('Missing ' + prop + ' (required).')
           // return({error:'Missing ' + prop + ' (required).'});
        } else {
            return;
        }
    }*/
...
User.login = function (props, callback) {
    ...
    var query = [
        'MATCH (n:Personne) WHERE n.username = '" + username + "' AND n.password = '" + password + "' RETURN id(n)',
        'RETURN user',
    ].join('\n');
