...
describe('unauthorized()', function () {
    ...
    it('returns a WWW-Authenticate header when passed a scheme', function (done) {
        ...
        var err = Boom.unauthorized('boom', 'Test.js');
        expect(err.output.statusCode).to.equal(401);
        expect(err.output.headers['WWW-Authenticate']).to.equal('Test.js error="boom"');
        done();
    });

    it('returns a WWW-Authenticate header set to the schema array value', function (done) {

        var err = Boom.unauthorized(null, ['Test.js','one','two']);
        expect(err.output.statusCode).to.equal(401);
        expect(err.output.headers['WWW-Authenticate']).to.equal('Test.js, one, two');
        done();
    });

    it('returns a WWW-Authenticate header when passed a scheme and attributes', function (done) {

        var err = Boom.unauthorized('boom', 'Test.js', { a: 1, b: 'something', c: null, d: 0 });
        expect(err.output.statusCode).to.equal(401);
        expect(err.output.headers['WWW-Authenticate']).to.equal('Test.js a="1", b="something", c="", d="0", error="boom"');
    ...
    it('returns a WWW-Authenticate header when passed attributes, missing error', function (done) {
        ...
        var err = Boom.unauthorized(null, 'Test.js', { a: 1, b: 'something', c: null, d: 0 });
        expect(err.output.statusCode).to.equal(401);
        expect(err.output.headers['WWW-Authenticate']).to.equal('Test.js a="1", b="something", c="", d="0"');
